...
onload = function () {
    ...
    //It is map i.e structure in which name is mapped to function
    const tools = {
        //We have to upload the pic
        "upload" : function () {
            ...
            upload.onchange = function() {
                const img = new Image();
                img.onload = () => {     //When image is set on src
        ...
        "greyscale" : function(){
            ...
            //Setting the same value of R G B i.e R==G==B
            //This Formula is scientifically made
            for(let i=0;i<rows;i++){
                for(let j=0;j<cols;j++){
                    let pixel = image[i][j];
    ...
    //Toolbar is ref to html section
    //After clicking button i.e when ever we get the call we call func
    ...
    //Each pixel of image is having 3 paramenter i.e H(Height), W(Width), 4_Parameters(Red,Green,Blue,Alpha); Alpha -> Opaquicity
